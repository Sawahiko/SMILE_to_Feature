...
final4 = final3[["SMILES","A_Pred", "B_Pred", "C_Pred", "A", "B", "C", "T"]]
def cb(x):
    x1 = x.strip('][').split(', ')
...
final4["ln_Psat_Actual (Pa)"] = final4.apply(lambda x: Psat_cal(x["T"], 
                                         x["A"], x["B"], x["C"]), axis=1)
final4["ln_Psat_Pred (Pa)"] = final4.apply(lambda x: Psat_cal(x["T"], 
                                         x["A_Pred"], x["B_Pred"], x["C_Pred"]), axis=1)
final4["RMSE"] = final4.apply(lambda x: mean_squared_error(x["ln_Psat_Actual (Pa)"], x["ln_Psat_Pred (Pa)"],squared=False), axis=1)
...
for i in range(len(final3)):
    ...
    count = do_plot(rmse_predict, True, count, thereshold=thereshold_input, is_second_cond=True)
