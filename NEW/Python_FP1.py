...
#con        df = pd.read_excel("../Data.xlsx",sheet_name="560point")
        ...
        #print(X_data_use["molecule"][1])
        #fp1=rdMolDescriptors.GetHashedAtomPairFingerprintAsBitVect(X_data_use["molecule"][1])
        #arr_fp1=np.array(fp1.GetOnBits())
        
        X_data_use["fp_AtomPairs"] = X_data_use["molecule"].apply(lambda x: rdMolDescriptors.GetHashedAtomPairFingerprintAsBitVect(
                x, 
                #radius=MF_radius, 
                nBits=2**11
                ))
        ...
        for i in range(X_data_use.shape[0]):
            ...
            array = np.array(X_data_use["fp_AtomPairs"][i])
            datafram_i = pd.DataFrame(array)
            datafram_i = datafram_i.T
            X_data_fp.append(datafram_i)
        x_data_fp = pd.concat(X_data_fp, ignore_index=False)
